...
{
  TST_B_TST(ret, off + 1,
            vstr_srch_chr_fwd(t1, 1, t1->len, 'a') != lens_fwd[0]);
  TST_B_TST(ret, off + 2,
            vstr_srch_chr_rev(t1, 1, t1->len, 'a') != lens_fwd[3]);
  TST_B_TST(ret, off + 3,
            vstr_srch_chr_fwd(t1, 1, t1->len, 'x') != lens_fwd[1]);
  TST_B_TST(ret, off + 4,
   ...
   * the rest minus 1 */
  TST_B_TST(ret, off + 7,
            vstr_srch_chr_fwd(t1, lens_fwd[0], t1->len - lens_fwd[0],
                              'a') != lens_fwd[0]);
  TST_B_TST(ret, off + 8,
            vstr_srch_chr_rev(t1, lens_fwd[0], t1->len - lens_fwd[0],
                              'a') != lens_fwd[3]);
}
...
int tst(void)
{
  ...
  TST_B_TST(ret, 25,
            vstr_srch_chr_rev(s1, s1->len - 6, 7, 'b') != (s1->len - 2));
  TST_B_TST(ret, 25,
            vstr_srch_chr_rev(s4, s3->len - 6, 7, 'b') != (s3->len - 2));
  TST_B_TST(ret, 25,
            vstr_srch_chr_rev(s3, s4->len - 6, 7, 'b') != (s4->len - 2));
