...
int tst(void)
{
  ...
  if (MFAIL_NUM_OK)
  {
    ...
    vstr_free_spare_nodes(NULL,     VSTR_TYPE_NODE_PTR, 1000);
    
    TST_B_TST(ret,  5, !tst_mfail_num(1));
    TST_B_TST(ret,  6, vstr_dup_cstr_ptr(s3->conf, buf));
    TST_B_TST(ret,  7, !tst_mfail_num(2));
    TST_B_TST(ret,  8, vstr_dup_cstr_ptr(s3->conf, buf));
    TST_B_TST(ret,  9, !tst_mfail_num(1));
    TST_B_TST(ret, 10, vstr_dup_cstr_ptr(NULL, buf));
    TST_B_TST(ret, 11, !tst_mfail_num(2));
    TST_B_TST(ret, 12, vstr_dup_cstr_ptr(NULL, buf));
  }
